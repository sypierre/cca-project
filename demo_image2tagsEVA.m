...
simI2T = @(x,t,W) (x*W{1})*(t*W{2})' ./...
    ( norm(x*W{1}) * sum( (t*W{2}).*(t*W{2}) ,2 )' );

simT2I = @(x,t,W ) (t*W{2})*(x*W{1})' ./...
    ( norm(t*W{2} ) * sum( (x*W{1}).*(x*W{1}),2 )' );

simI2I = @(xq,x,W ) (xq*W{1})*(x*W{1})' ./...
    ( norm(xq*W{1} ) * sum( (x*W{1} ).*(x*W{1} ) ,2 )' );
...
if ~ opt.EVA
[requestname, abslines] = request_name(nsst,inria_imgdir, inria_objfi, opt);
NN = 1;

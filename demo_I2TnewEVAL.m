...
if 1 % ~ exist('rel_test')
    ...
    rel_testeval = rel_testeval( randi(length(rel_testeval),1,min(2000,length(rel_testeval)))   );
...
%     plot_ccaNEW(nss,Z, Z, opt, 1, semantic);
    ...
    RETmatrix = simI2Tval( Zeval{1}(:,1:end-1), Zbase(:,1:end-1) );
    ...
    for cc = 1 : length(cls)
        clid = cls(cc); 
        % find clid in Z{2}
        vallines = find( inria_objfi(Zeval{1}(:,end) ,1 ) == clid  );
